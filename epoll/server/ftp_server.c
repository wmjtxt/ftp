...
//					printf("rdset\n");
					...
					if(logflag==0)
					{
						...
						recv(pcur->new_fd,&recvlen,sizeof(int),0);
						recv(pcur->new_fd,pname,recvlen,0);//接收用户名
						recvlen=0;
						memset(passwd,0,sizeof(passwd));
						recv(pcur->new_fd,&recvlen,sizeof(int),0);
						recv(pcur->new_fd,passwd,recvlen,0);//接收密码
                        printf("pname=%s\n",pname);
                        printf("passwd=%s\n",passwd);
						logflag=login(pname,passwd);
						login_syslog(pname,time(NULL));//cat /var/log/syslog
						recvlen=0;
						send(pcur->new_fd,&logflag,sizeof(int),0);
						continue;
					}
